#!/bin/bash
# setup_server.sh — Executar no VPS (${SECRET_MQTT_BROKER})
...
if ! sudo test -f /etc/letsencrypt/live/${SECRET_MQTT_BROKER}/fullchain.pem; then
    echo "  Certificado não encontrado. Para obter um, execute:"
    echo "  sudo certbot --nginx -d ${SECRET_MQTT_BROKER}"
else
...
echo "  1. Definir SECRET_MQTT_BROKER, SECRET_MQTT_USERNAME, SECRET_MQTT_PASSWORD no ambiente de build"

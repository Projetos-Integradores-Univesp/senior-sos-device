...
public:
    ...
    // =========================================================================
    // LUT OCV → SOC para LiPo NMC 3.7 V nominal
    //
    // Baseada em curvas OCV a baixa taxa (<0.1C) a 25°C — média carga/descarga
    // Cutoff 3.00 V = 0 %  (operação mínima observada no XIAO ESP32S3)
    // Full   4.20 V = 100 %
    //
    // O MAX17048 lê a tensão com precisão de ±7.5 mV (muito superior ao SOC
    // interno, que depende dos parâmetros ModelGauge não calibrados para esta
    // bateria). Por isso usamos a tensão medida pelo chip e interpolamos na LUT.
    //
    // Interpolação linear entre os pontos garante resolução ~0.5–1 %/mV na
    // região plana (3.50–3.90 V) e até 5 %/mV na queda final (<3.3 V).
    //
    // Fonte: literatura NMC/LiPo OCV-SOC (Baccouche 2017, Zhang 2018)
    //        ajustada para cutoff 3.0 V conforme testes do ElderGuard.
    // =========================================================================

    struct OcvPoint { float v; uint8_t soc; };

    static int voltageToSOC(float voltage) {
        // LUT: 21 pontos de 0% a 100%, tensão decrescente
        static const OcvPoint LUT[] = {
            {4.200f, 100}, {4.165f, 95}, {4.130f, 90}, {4.090f, 85},
            {4.050f,  80}, {4.000f, 75}, {3.950f, 70}, {3.900f, 65},
            {3.860f,  60}, {3.820f, 55}, {3.790f, 50}, {3.760f, 45},
            {3.730f,  40}, {3.705f, 35}, {3.680f, 30}, {3.650f, 25},
            {3.600f,  20}, {3.530f, 15}, {3.420f, 10}, {3.270f,  5},
            {3.000f,   0},
        };
        static const int N = sizeof(LUT) / sizeof(LUT[0]);

        // Fora dos extremos: fixar em 100% ou 0%
        if (voltage >= LUT[0].v)    return 100;
        if (voltage <= LUT[N-1].v)  return 0;

        // Busca linear (N=21 — custo desprezível, evita dependência de stdlib)
        for (int i = 0; i < N - 1; i++) {
            if (voltage >= LUT[i+1].v) {
                // Interpolação linear entre ponto i e i+1
                float dv   = LUT[i].v   - LUT[i+1].v;
                float frac = (voltage   - LUT[i+1].v) / dv;
                float soc  = LUT[i+1].soc + frac * (LUT[i].soc - LUT[i+1].soc);
                int result = (int)(soc + 0.5f);
    ...
    static int batteryPercent() {
        // Usa tensão medida + LUT OCV-SOC em vez do SOC interno do MAX17048
        // (os parâmetros ModelGauge padrão são imprecisos para esta bateria)
        float v = readBatteryVoltage();
        if (v < 0.5f) {
            // Tensão inválida — tentar SOC interno como fallback
            float soc = readBatterySOC();
            return (soc < 0) ? -1 : (int)(soc + 0.5f);
        }
        return voltageToSOC(v);
    }
    ...
    static void enterDeepSleep() {
        ...
        // Limpar interrupção pendente do IMU (INT ativo-LOW travado)
        // Suporta MPU6500 (INT_STATUS em 0x3A) e BMI160 (INT_STATUS_0 em 0x1C).
        // O IMU_ADDR e IMU_INT_STATUS_REG são selecionados pelo include ativo.
        pinMode(IMU_INT, INPUT_PULLUP);
        if (digitalRead(IMU_INT) == LOW) {
            out.println("[PWR] IMU INT LOW — limpando...");

#ifdef IMU_BMI160_H
            // BMI160: modo non-latched — leitura de INT_STATUS_0 libera o pino
            const uint8_t imu_addr    = BMI_ADDR;
            const uint8_t int_status  = 0x1C;   // BMI_REG_INT_STATUS_0
#else
            // MPU6500: leitura de INT_STATUS (0x3A) libera o latch
            const uint8_t imu_addr    = MPU6500_ADDR;
            ...
            out.printf("[PWR] IMU INT: %s\n",
                digitalRead(IMU_INT) == HIGH ? "limpo" : "ainda LOW");
        }
